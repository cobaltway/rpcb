...
module.exports = async function ({ characterID, kinkID, name, description, field }, user) {
  ...
  if (!requestedCharacter.creator._id.equals(user._id) && !user.canAccessKeystone) throw new Error('NOT_AUTHORIZED');
  ...
  if (kinkID) {
    ...
    else requestedCharacter.kinks[field].push(kinkToAdd._id);
  }
  else if (field !== 'none') {
    keystone.truthy({ name });
    const newKink = new (keystone.list('Kink')).model({ name, creator: user, custom: true });
    if (description !== undefined) newKink.description.md = description;
  ...
  return keystone.format(await requestedCharacter.save());
};
